...
void acc_call(pci_bar_handle_t pci_bar_handle,XInference_net InstancePtr,
    int acc_param_conv[],uint64_t CONV_CORE_PARAM,
    int R_out,int C_out){
    for(int i=0;i<R_out;i+=Tr){
        for(int j=0;j<C_out;j+=Tc){
            acc_param_conv[5]=i;
            acc_param_conv[6]=j;
            Fill_param(pci_bar_handle, CONV_CORE_PARAM, acc_param_conv, 16);
...
void cdma_cp(pci_bar_handle_t pci_bar_handle,int feature_num,int feature_width,uint32_t src_address, uint32_t dst_address, uint32_t dst_buf_width){
    for(int i = 0;i < feature_num;i++){
        for(int j = 0;j < feature_width;j++){
            set_cdma(pci_bar_handle,src_address+feature_num*0x00001000+feature_width*0x00000080,0x00000000,dst_address+feature_num*0x00001000+feature_width*0x00000080,0x00000000,dst_buf_width);
        }
    }
}

void set_pwrite(int rc, int fd, float *write_buffer, size_t buffer_size, uint64_t dst_address){
...
/* TODO: change this function into parameter based function call, do not directly use fixed value in it */
